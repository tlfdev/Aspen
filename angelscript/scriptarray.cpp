        ...
        else if ((flags & asOBJ_REF))
        {
            ...
            if (!found)
            {
                ...
                snprintf(buffer, sizeof(buffer), "The subtype '%s' has no default factory",
                         subtype ? subtype->GetEngine()->GetTypeDeclaration(subtype->GetTypeId()) : "UNKNOWN");
...
static void RegisterScriptArray_Native(asIScriptEngine* engine)
{
    ...
    assert(r >= 0);
    // The token 'if_handle_then_const' tells the engine that if the type T is a handle, then it should refer to a
    // read-only object
    ...
    r = engine->RegisterObjectMethod("array<T>", "int findByRef(const T&in if_handle_then_const value) const",
                                     asMETHODPR(CScriptArray, FindByRef, (const void*) const, int), asCALL_THISCALL);
    assert(r >= 0);
...
#define COMPARE(T) *((const T*)a) < *((const T*)b)
            ...
            default:
                return COMPARE(signed int); // All enums fall in this case. TODO: update this when enums can have
            ...
            if (ctx)
            {
                ...
                if (cache && cache->eqFuncReturnCode == asMULTIPLE_FUNCTIONS)
                    ...
                    sprintf_s(tmp, 512, "Type '%s' has multiple matching opEquals or opCmp methods",
                              subType->GetName());
#else
                    snprintf(tmp, sizeof(tmp), "Type '%s' has multiple matching opEquals or opCmp methods",
                             subType->GetName());
#endif
                else
#if defined(_MSC_VER) && _MSC_VER >= 1500 && !defined(__S3E__)
                    sprintf_s(tmp, 512, "Type '%s' does not have a matching opEquals or opCmp method",
                              subType->GetName());
#else
                    snprintf(tmp, sizeof(tmp), "Type '%s' does not have a matching opEquals or opCmp method",
                             subType->GetName());
#endif
...
void CScriptArray::Sort(asIScriptFunction* func, asUINT startAt, asUINT count)
{
    ...
    // TODO: Security issue: If the array is accessed from the callback while the sort is going on the result may be
    // unpredictable
...
static void RegisterScriptArray_Generic(asIScriptEngine* engine)
{
    ...
    r = engine->RegisterObjectMethod("array<T>", "uint opForBegin() const", asFUNCTION(ScriptArray_opForBegin_Generic),
                                     asCALL_GENERIC);
    assert(r >= 0);
    r = engine->RegisterObjectMethod("array<T>", "bool opForEnd(uint) const", asFUNCTION(ScriptArray_opForEnd_Generic),
                                     asCALL_GENERIC);
    assert(r >= 0);
    r = engine->RegisterObjectMethod("array<T>", "uint opForNext(uint) const",
    ...
    assert(r >= 0);
    r = engine->RegisterObjectMethod("array<T>", "void removeAt(uint index)", asFUNCTION(ScriptArrayRemoveAt_Generic),
                                     asCALL_GENERIC);
    assert(r >= 0);
