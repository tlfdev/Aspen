...
// BNF:6: PRIMTYPE      ::= 'void' | 'int' | 'int8' | 'int16' | 'int32' | 'int64' | 'uint' | 'uint8' | 'uint16' |
...
// BNF:17: IDENTIFIER    ::= [A-Za-z_][A-Za-z0-9_]*            // single token:  starts with letter or _, can include
// any letter and digit, same as in C++
...
// BNF:3: PARAMLIST     ::= '(' ('void' | (TYPE TYPEMOD ('...' | IDENTIFIER? ('=' EXPR)?) (',' TYPE TYPEMOD ('...' |
// IDENTIFIER? ('=' EXPR)?))*))? ')'
...
// BNF:18: COMMENT       ::= ('//'[^#x0A]*) | ('/*'[^*]*'*/')  // single token:  starts with // and ends with new line
// or starts with /* and ends with */ BNF:18: WHITESPACE    ::= [ #x09#x0A#x0D]+                  // single token:
// spaces, tab, carriage return, line feed, and UTF8 byte-order-mark
...
// BNF:17: NUMBER        ::= [0-9]+("."[0-9]+)?                // single token:  includes integers and real numbers,
// same as C++ BNF:17: STRING        ::= '"' ("\". | [^"#x0D#x0A\\])* '"'   // single token:  single quoted ', double
// quoted ", or heredoc multi-line string """ BNF:17: BITS          ::= '0'[bBoOdDxX][0-9A-Fa-f]+         // single
// token:  binary 0b or 0B, octal 0o or 0O, decimal 0d or 0D, hexadecimal 0x or 0X
...
// BNF:12: LAMBDA        ::= 'function' '(' ((TYPE TYPEMOD)? IDENTIFIER? (',' (TYPE TYPEMOD)? IDENTIFIER?)*)? ')'
// STATBLOCK
...
asCScriptNode* asCParser::ParseArgList(bool withParenthesis)
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            // It also avoids conflict with expressions to that creates anonymous objects initialized with lists, i.e.
            // type = {...} The alternate syntax: arg = expr, is supported to provide backwards compatibility
            // with 2.29.0
...
// BNF:11: EXPRPOSTOP    ::= ('.' (FUNCCALL | IDENTIFIER)) | ('[' (IDENTIFIER ':')? ASSIGN (',' (IDENTIFIER ':')?
// ASSIGN)* ']') | ARGLIST | '++' | '--'
...
bool asCParser::IsOperator(int tokenType)
{
    if (tokenType == ttPlus || tokenType == ttMinus || tokenType == ttStar || tokenType == ttSlash ||
        ...
        tokenType == ttBitShiftRight || tokenType == ttBitShiftRightArith || tokenType == ttIs || tokenType == ttNotIs)
        return true;
...
// BNF:0: SCRIPT        ::= (IMPORT | ENUM | TYPEDEF | CLASS | MIXIN | INTERFACE | FUNCDEF | VIRTPROP | VAR | FUNC |
// NAMESPACE | USING | ';')*
asCScriptNode* asCParser::ParseScript(bool inBlock)
{
    ...
    for (;;)
    {
        while (!isSyntaxError)
        {
            ...
            // Optimize by skipping tokens 'shared', 'external', 'final', 'abstract' so they don't have to be checked in
            // every condition
            sToken t1 = tStart;
...
// BNF:1: ENUM          ::= ('shared' | 'external')* 'enum' IDENTIFIER (';' | ('{' IDENTIFIER ('=' EXPR)? (','
// IDENTIFIER ('=' EXPR)?)* '}'))
...
// BNF:1: FUNC          ::= ('shared' | 'external')* ('private' | 'protected')? (((TYPE '&'?) | '~'))? IDENTIFIER
// PARAMLIST 'const'? FUNCATTR (';' | STATBLOCK)
...
// BNF:1: VIRTPROP      ::= ('private' | 'protected')? TYPE '&'? IDENTIFIER '{' (('get' | 'set') 'const'? FUNCATTR
// (STATBLOCK | ';'))* '}'
...
// BNF:1: INTERFACE     ::= ('external' | 'shared')* 'interface' IDENTIFIER (';' | ((':' IDENTIFIER (',' IDENTIFIER)*)?
// '{' (VIRTPROP | INTFMTHD)* '}'))
...
// BNF:1: CLASS         ::= ('shared' | 'abstract' | 'final' | 'external')* 'class' IDENTIFIER (';' | ((':' IDENTIFIER
// (',' IDENTIFIER)*)? '{' (VIRTPROP | FUNC | VAR | FUNCDEF)* '}'))
...
// BNF:1: VAR           ::= ('private'|'protected')? TYPE IDENTIFIER (( '=' (INITLIST | EXPR)) | ARGLIST)? (','
...
// BNF:7: STATEMENT     ::= (IF | FOR | FOREACH | WHILE | RETURN | STATBLOCK | BREAK | CONTINUE | DOWHILE | SWITCH |
// EXPRSTAT | TRY)
