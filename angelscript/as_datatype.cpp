    ...
    else
    {
        ...
        else if (!isObjectHandle)
        {
            ...
            if ((!typeInfo ||
                 ...
                 (typeInfo->flags & asOBJ_NOHANDLE) || ((typeInfo->flags & asOBJ_SCOPED) && !acceptHandleForScope)))
                return -1;
...
int asCDataType::GetSizeOnStackDWords() const
{
    ...
    // TODO: bug: Registered value types are also stored on the stack. Before changing though, check how
    // GetSizeOnStackDWords is used
    //            When called to determine size of type as parameter then it is correct, as objects are implicitly
    //            passed by reference in AngelScript To correct this it would be necessary to know if the method is
    //            called for a parameter, or for a local variable
