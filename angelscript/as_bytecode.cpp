    ...
    while (curr)
    {
        if (asBCInfo[curr->op].type == asBCTYPE_wW_rW_rW_ARG)
        {
            ...
            InsertIfNotExists(vars, curr->wArg[2]);
                 ...
                 asBCInfo[curr->op].type == asBCTYPE_rW_W_DW_ARG || asBCInfo[curr->op].type == asBCTYPE_rW_DW_DW_ARG)
        {
            InsertIfNotExists(vars, curr->wArg[0]);
        }
...
void asCByteCode::OptimizeLocally(const asCArray<int>& tempVariableOffsets)
{
    ...
    // TODO: runtime optimize: VAR + GET... should be optimized if the only instructions between them are trivial, i.e.
    // no
    ...
    // TODO: runtime optimize: A single bytecode for incrementing a variable, comparing, and jumping can probably
    // improve
    //                         loops a lot. How often do these loops really occur?

    // TODO: runtime optimize: Need a bytecode BC_AddRef so that BC_CALLSYS doesn't have to be used for this trivial
    // call
    ...
    while (instr)
    {
        ...
        else if (currOp == asBC_PopPtr)
        {
            ...
            else if (instr && (instr->op == asBC_PshRPtr || instr->op == asBC_PSF || instr->op == asBC_VAR ||
            ...
            else if (instr && instr->op == asBC_FREE)
            {
                ...
                // TODO: runtime optimize: The FREE instruction shouldn't be necessary. STOREOBJ should free the
...
void asCByteCode::Optimize()
{
    ...
    //                         This is especially useful with the factory stubs used for template types and script
    //                         classes.
        ...
        while (curr)
        {
            ...
            if (curr->op == asBC_JMP)
            {
                ...
                break;
                     ...
                     curr->op == asBC_JP || curr->op == asBC_JNP || curr->op == asBC_JLowZ || curr->op == asBC_JLowNZ)
            {
                // Find the destination. If it cannot be found it is because we're doing a localized
                // optimization and the label hasn't been added to the final bytecode yet
...
bool asCByteCode::IsTempRegUsed(asCByteInstruction* curr)
{
    ...
    while (curr->next)
    {
        ...
        if (curr->op == asBC_INCi || curr->op == asBC_INCi16 || curr->op == asBC_INCi8 || curr->op == asBC_INCf ||
            ...
            curr->op == asBC_JMPP || // TODO: JMP and JMPP cannot be said to read the temporary register. Need to follow
                                     // the branch to determine what happens next
            curr->op == asBC_JMP)
            return true;
        ...
        if (curr->op == asBC_CALL || curr->op == asBC_PopRPtr || curr->op == asBC_CALLSYS || curr->op == asBC_CALLBND ||
            ...
            curr->op == asBC_TP || curr->op == asBC_TNP || curr->op == asBC_CMPi || curr->op == asBC_CMPu ||
            curr->op == asBC_CMPf || curr->op == asBC_CMPd || curr->op == asBC_CMPIi || curr->op == asBC_CMPIu ||
            curr->op == asBC_CMPIf || curr->op == asBC_LoadThisR || curr->op == asBC_LoadRObjR ||
...
void asCByteCode::ExtractObjectVariableInfo(asCScriptFunction* outFunc)
{
    ...
    while (instr)
    {
        ...
        else if (instr->op == asBC_VarDecl)
        {
            ...
            // This is used for identifying if handles and objects on the heap should be cleared upon catching an
            // exception Only extract this info if there is a try/catch block in the function, so we don't use up
            // unnecessary space
                ...
                switch (instr->op)
                {
                    ...
                    case asBC_TYPEID:
                        fprintf(file, "   %-8s 0x%x          '%s'\n", asBCInfo[instr->op].name,
                                (asUINT)*ARG_DW(instr->arg), engine->GetTypeDeclaration((int)*ARG_DW(instr->arg)));
                        break;
