...
void asCConfigGroup::AddReferencesForType(asCScriptEngine* engine, asCTypeInfo* type)
{
        ...
        !generatedTemplateInstances.Exists(CastToObjectType(type)))
    {
        ...
        // This is needed when a template instance refers to another template instance, and thus is created at the same
        // time. Without adding the reference to the referred template instance, it may be deleted when the module
        // deletes the top template instance.
        ...
        for (asUINT n = 0; n < ot->beh.constructors.GetLength(); n++)
        {
            ...
            for (asUINT p = 0; p < f->parameterTypes.GetLength(); p++)
            {
                ...
                if (ti && (ti->flags & asOBJ_TEMPLATE) && engine->generatedTemplateTypes.Exists(CastToObjectType(ti)) &&
