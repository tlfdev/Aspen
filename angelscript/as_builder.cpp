...
class asCCompGlobVarType : public asIFilter
{
  ...
  private:
    // The assignment operator is required for MSVC9, otherwise it will complain that it is not possible to auto
    ...
    if (numErrors == 0)
    {
        ...
        for (n = 0; n < interfaceDeclarations.GetLength(); n++)
        {
            ...
            while (node)
            {
                ...
                {
                    node->DisconnectParent();
                    RegisterScriptFunctionFromNode(node, decl->script, CastToObjectType(decl->typeInfo), true, false, 0,
        ...
        for (n = 0; n < classDeclarations.GetLength(); n++)
        {
            ...
            while (node)
            {
                ...
                {
                    node->DisconnectParent();
                    RegisterScriptFunctionFromNode(node, decl->script, CastToObjectType(decl->typeInfo), false, false,
            ...
            if (!decl->isExistingShared)
            {
                ...
                // Add the default copy constructor if needed (only if no other constructor with single parameter is
                // explicitly defined)
...
void asCBuilder::CompileFunctions()
{
    ...
    for (asUINT n = 0; n < functions.GetLength(); n++)
    {
        ...
        if (current->node &&
            !(current->node->nodeType == snStatementBlock || current->node->lastChild->nodeType == snStatementBlock))
            continue;
                                         ...
                                         asCObjectType** outParentClass)
{
    ...
    func->returnType =
        CreateDataTypeFromNode(node->firstChild, &source, objType ? objType->nameSpace : ns, true,
                               parentClass ? parentClass : objType, true, 0, isTemplate ? &func->templateSubTypes : 0);
                                        ...
                                        bool isProperty, bool isVirtualProperty)
{
    ...
    // Don't do this when the check is for a virtual property, as it is allowed to have multiple overloads for virtual
    // properties Properties are allowed to have the same name as virtual properties
    ...
    // If there is a namespace at the same level with the same name as the class, then need to check for conflicts with
    // symbols in that namespace too
    // TODO: When classes can have static members, the code should change so that class name cannot be the same as a
    // namespace
    asCString scope;
...
int asCBuilder::ValidateVirtualProperty(asCScriptFunction* func)
{
    ...
    if (found)
    {
        ...
        if (!getType.IsEqualExceptRefAndConst(setType) && !((getType.IsObjectHandle() && !setType.IsObjectHandle()) &&
...
void asCBuilder::CompleteFuncDef(sFuncDef* funcDef)
{
    ...
    if (func->IsShared())
    {
        for (asUINT n = 0; n < engine->funcDefs.GetLength(); n++)
        {
            ...
            if (fdt2->name == fdt->name && fdt2->nameSpace == fdt->nameSpace && fdt2->parentClass == fdt->parentClass &&
    ...
    if (funcTraits.GetTrait(asTRAIT_EXTERNAL) && found)
        module->m_externalTypes.PushLast(
            engine->scriptFunctions[module->m_funcDefs[funcDef->idx]->GetFuncdefSignature()->GetId()]->funcdefType);
}
...
void asCBuilder::CompileClasses(asUINT numTempl)
{
        ...
        while (toValidate.GetLength() > 0)
        {
            ...
            for (n = 0; n < ot->properties.GetLength(); n++)
            {
                ...
                if( dt.IsTemplate() )
                {
                    ...
                    // TODO: Just because the subtype is not a handle doesn't mean the template will actually instance
                the object
                    //       this it shouldn't automatically raise an error for this, e.g. weakref<Object> should be
                legal as member
                    //       of the Object class
                    asCDataType sub = dt;
        ...
        if (funcDesc != asUINT(-1) && functions[funcDesc]->node == 0)
        {
            ...
            if (objType->derivedFrom)
            {
                ...
                else
                {
                    ...
                    // TODO: Should allow it anyway, but the copy constructor should be generated as taking a non-const
                    // reference
    ...
    //
    //                         Observe, that doing this would conflict with another potential future feature, which is
    //                         to allow incremental builds, i.e. allow application to add or replace classes in an
                ...
                else
                {
                    ...
                    if (dt.IsObjectHandle())
                    {
                        ...
                        if (prop->flags & asOBJ_SCRIPT_OBJECT)
                        {
                            ...
                            if (prop->flags & asOBJ_NOINHERIT)
                            {
                                for (asUINT sp = 0; sp < prop->properties.GetLength(); sp++)
                                {
                                    ...
                                    if (sdt.IsObject())
                                    {
                                        ...
                                        {
                                            // TODO: runtime optimize: If the handle is again to a final class, then we
                                            // can recursively check if the circular reference can occur
                                        ...
                                        {
                                            // TODO: runtime optimize: Just because the member type is a potential
                                            // circle doesn't mean that this one is.
                                            //                         Only if the object is of a type that can
                                            //                         reference this type, either directly or
                                            //                         indirectly
                                            gc = true;
                                            break;
                                        }
                            ...
                            {
                                // Assume it is garbage collected as it is not known at compile time what might inherit
                                // from this type
                                gc = true;
                                break;
                            }
                        ...
                        else if (prop->flags & asOBJ_GC)
                        {
                            ...
                            // TODO: runtime optimize: Just because an application registered class is garbage
                            // collected, doesn't mean it
                            //                         can form a circular reference with this script class. Perhaps
                            //                         need a flag to tell if the script classes that contains the type
                    ...
                    {
                        // TODO: runtime optimize: Just because the member type is a potential circle doesn't mean that
                        // this one is.
                        //                         Only if the object is of a type that can reference this type, either
...
void asCBuilder::IncludeMethodsFromMixins(sClassDeclaration* decl)
{
    ...
    while (node && node->nodeType == snIdentifier)
    {
        ...
        if (mixin)
        {
            ...
            // Possible keywords 'final' and 'shared' are removed in RegisterMixinClass so we don't need to worry about
            // those here
            while (n && n->nodeType == snIdentifier)
                n = n->next;
            ...
            while (n)
            {
                if (n->nodeType == snFunction)
                {
                    ...
                    RegisterScriptFunctionFromNode(copy, mixin->script, CastToObjectType(decl->typeInfo), false, false,
                                                   mixin->ns, false, true);
                }
...
void asCBuilder::AddDefaultCopyConstructor(asCObjectType* objType, asCScriptCode* file)
{
    ...
    module->AddScriptFunction(file->idx, 0, funcId, objType->name, returnType, parameterTypes, parameterNames,
                              inOutFlags, defaultArgs, false, objType, false, asSFunctionTraits(), objType->nameSpace);
    ...
    if (asSUCCESS == r)
    {
        ...
        while (tmp && tmp->nodeType == snIdentifier)
        {
            ...
            else
            {
                ...
                // Set the index to a negative to allow compiler to diferentiate from ordinary global var when compiling
                // the initialization
                gvar->index = -1;
                gvar->isCompiled = false;
                                               ...
                                               bool isExistingShared, bool isMixin, sClassDeclaration* decl)
{
    ...
    return RegisterScriptFunction(node, file, objType, isInterface, isGlobalFunction, ns, isExistingShared, isMixin,
                                  name, returnType, parameterNames, parameterTypes, inOutFlags, defaultArgs, funcTraits,
                                  decl);
}
                                              ...
                                              const asCString& name, asSNameSpace* ns, bool isShared)
{
    ...
    int r =
        RegisterScriptFunction(args, file, 0, 0, true, ns, false, false, funcName, funcDef->returnType, parameterNames,
                               funcDef->parameterTypes, funcDef->inOutFlags, defaultArgs, traits, 0);
    if (r < 0)
        return 0;
    ...
    if (isExistingShared)
    {
        ...
        else
        {
            for (asUINT n = 0; n < objType->methods.GetLength(); n++)
            {
                ...
                if (func->name == name && func->IsSignatureExceptNameEqual(returnType, parameterTypes, inOutFlags,
                                                                           objType, funcTraits.GetTrait(asTRAIT_CONST)))
                {
    ...
    if (objType)
    {
        ...
        if (funcTraits.GetTrait(asTRAIT_CONSTRUCTOR))
        {
            ...
            module->AddScriptFunction(file->idx, engine->scriptFunctions[funcId]->scriptData->declaredAt, factoryId,
                                      ...
                                      false, funcTraits, objType->nameSpace);
                                        ...
                                        bool isExistingShared)
{
    ...
    while (node)
    {
        ...
        if (name != "")
        {
            ...
            {
                // TODO: getset: If no implementation is supplied the builder should provide an automatically generated
                // implementation
                //               The compiler needs to be able to handle the different types, primitive, value type, and
                //               handle The code is also different for global property accessors
        ...
        {
            if (!isExistingShared)
                RegisterScriptFunction(funcNode ? funcNode : node, file, objType, isInterface, isGlobalFunction, ns,
                                       false, false, name, returnType, paramNames, paramTypes, paramModifiers,
                                       defaultArgs, funcTraits, 0);
            else
            {
...
void asCBuilder::GetFunctionDescriptions(const char* name, asCArray<int>& funcs, asSNameSpace* ns)
{
    ...
    // TODO: optimize: Linear search: This is probably not that critial. Also bindInformation will probably be removed
    // in near future
                                             ...
                                             asCScriptCode* script)
{
    ...
    if (scope != "")
    {
        // If searching with a scope informed, then the node and script must also be informed for potential error
        // reporting
        asASSERT(errNode && script);

        // If the scope contains ::identifier, then use the last identifier as the class name and the rest of it as the
        // namespace
    ...
    for (asUINT n = 0; n < objectType->methods.GetLength(); n++)
    {
        ...
        if (func->name == name && (!objIsConst || func->IsReadOnly()) && (func->accessMask & module->m_accessMask))
        {
                                               ...
                                               asCScriptNode** next, asCObjectType** objType, bool* isExplicitNs)
{
    ...
    while (sn)
    {
        ...
        else
        {
            ...
            bool ok =
                FindObjectTypeOrMixinInNsHierarchy(templateName, ns, scope != "", sn->next, script, &templateType, 0);
    ...
    if (scope != originalScope && ns == 0)
        ns = GetNameSpaceByString(originalScope, implicitNs ? implicitNs : engine->nameSpaces[0], node, script, &ti,
                                  true);
            ...
            if (!visitedNamespaces.Exists(ns) || parentType)
            {
                ...
                if (ti && (!found || checkAmbiguousSymbols) && canContinue)
                {
                    ...
                    if (!module || (module->m_accessMask & ti->accessMask))
                    {
                        ...
                        {
                            // TODO: typedef: A typedef should be considered different from the original type (though
                            // with implicit conversions between the two) Create primitive data type based on object
                            // flags
            ...
            !(dt.GetTypeInfo() && (dt.GetTypeInfo()->flags & asOBJ_TEMPLATE_SUBTYPE)))
        {
            ...
            if (!dt.IsObject() || dt.IsObjectHandle() ||
                !((dt.GetTypeInfo()->flags & asOBJ_NOCOUNT) ||
                  (CastToObjectType(dt.GetTypeInfo())->beh.addref && CastToObjectType(dt.GetTypeInfo())->beh.release)))
