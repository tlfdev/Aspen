...
int asCGarbageCollector::ReportAndReleaseUndestroyedObjects()
{
    ...
    for (asUINT n = 0; n < gcOldObjects.GetLength(); n++)
    {
        ...
        if (gcObj.type->name == "$func")
        {
            ...
            // parameter list already so the declaration would only be misleading We need to show the function type too
            // as for example delegates do not have a name
            msg.Format(TXT_PREV_FUNC_IS_NAMED_s_TYPE_IS_d, reinterpret_cast<asCScriptFunction*>(gcObj.obj)->GetName(),
