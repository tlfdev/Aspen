...
#include "../include/aswrappedcall.h"
                           ...
                           asIStringFactory* stringFactory)
{
    ...
    while (pos < config.length())
    {
        ...
        else if (token == "objbeh")
        {
            ...
            {
                // TODO: How can we let the compiler register this? Maybe through a plug-in system? Or maybe by
                // implementing the callback as a script itself
        ...
        else if (token == "objprop")
        {
            ...
            {
                engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0, asMSGTYPE_ERROR,
                                     "Wrong value for composite indirect. Is it an old config version?");
                return -1;
            }
