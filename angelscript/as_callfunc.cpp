...
int PrepareSystemFunction(asCScriptFunction* func, asSSystemFunctionInterface* internal, asCScriptEngine* engine)
{
    ...
    else if (func->returnType.IsObject())
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (func->returnType.GetTypeInfo()->flags & asOBJ_APP_CLASS_ALLFLOATS)
                    {
        ...
        #ifdef AS_RISCV64
                        // TODO: There shouldn't be platform specific code in this file. Need to have a better way of
                        // controlling this
                        ...
                        {
                            // Since asOBJ_APP_ALIGN8 is not set we assume it is floats, and only 2 floats can be
                            // returned in registers In this case the object will not be split by members, and instead
                            // it will be returned in a0:a1
                            internal->hostReturnFloat = false;
                        }
            ...
            !func->parameterTypes[n].IsReference())
        {
            ...
            if (
                ...
                !(func->parameterTypes[n].GetTypeInfo()->flags &
                  (asOBJ_APP_PRIMITIVE | asOBJ_APP_FLOAT | asOBJ_APP_CLASS_ALLINTS | asOBJ_APP_CLASS_ALLFLOATS)))
            {
                engine->WriteMessage("", 0, 0, asMSGTYPE_INFORMATION, func->GetDeclarationStr().AddressOf());
...
// TODO: cleanup: Should have only a single CallSystemFunctionNative function signature. However changing it requires
// retesting all different native platforms :(
...
// retPointer - This points to a the memory buffer where the return object is to be placed, if the function returns the
// value in memory rather than in registers retQW2     - This output parameter should be used if the function returns a
// value larger than 64bits in registers secondObj  - This is the object pointer that the proxy method should invoke its
// method on when the call convention is THISCALL_OBJFIRST/LAST
//
// Return value:
//
...
// args       - This is the function arguments, which are packed as in AngelScript
// retPointer - This points to a the memory buffer where the return object is to be placed, if the function returns the
// value in memory rather than in registers retQW      - This output parameter should be used if the function returns a
// value in registers. This is a pointer to an array of QWORDs secondObj  - This is the object pointer that the proxy
// method should invoke its method on when the call convention is THISCALL_OBJFIRST/LAST
