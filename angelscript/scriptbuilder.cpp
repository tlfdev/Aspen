#include "../include/scriptbuilder.h"
    ...
    while (pos < modifiedScript.size())
    {
        ...
        if (token == "[")
        {
            ...
            if (token == "#" && (pos + 1 < modifiedScript.size()))
            {
                    ...
                    if (token == "include")
                    {
                        ...
                        if (t == asTC_VALUE && len > 2 && (modifiedScript[pos] == '"' || modifiedScript[pos] == '\''))
                        {
                            ...
                            if (p != string::npos)
                            {
                                ...
                                string str = "Invalid file name for #include; it contains a line-break: '" +
                                             includefile.substr(0, p) + "'";
                ...
                {
                    // Check for lines starting with #!, e.g. shebang interpreter directive. These will be treated as
                    // comments and removed by the preprocessor
                    if (modifiedScript[pos] == '!')
                    {
                        // Read until the end of the line
                        pos += len;
            ...
            else
            {
                ...
                if (it == classMetadataMap.end())
                {
                    classMetadataMap.insert(
                        map<int, SClassMetadata>::value_type(typeId, SClassMetadata(decl->parentClass)));
                ...
                if (func)
                    it->second.funcMetadataMap.insert(
                        map<int, vector<string>>::value_type(func->GetId(), decl->metadata));
            }
        }
...
int CScriptBuilder::ExtractDeclaration(int pos, string& name, string& declaration, int& type)
{
    ...
    do
    {
        ...
        t = engine->ParseToken(&modifiedScript[pos], modifiedScript.size() - pos, &len);
        token.assign(&modifiedScript[pos], len);
    } while (t == asTC_WHITESPACE || t == asTC_COMMENT || token == "private" || token == "protected" ||
             token == "shared" || token == "external" || token == "final" || token == "abstract");
            ...
            for (; pos < (int)modifiedScript.size();)
            {
                ...
                if (t == asTC_KEYWORD)
                {
                    ...
                    if ((token == "=" && !hasParenthesis) || token == ";")
                    {
                        if (hasParenthesis)
                        {
                            // The declaration is ambigous. It can be a variable with initialization, or a function
                            // prototype
                ...
                if (!hasParenthesis || nestedParenthesis > 0 || t != asTC_IDENTIFIER ||
                    (token != "final" && token != "override" && token != "delete" && token != "property"))
                    declaration += token;
