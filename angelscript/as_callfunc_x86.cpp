                                 ...
                                 void* retPointer, asQWORD& /*retQW2*/, void* secondObject)
{
    ...
    if (isThisCallMethod && (callConv >= ICC_THISCALL_OBJLAST && callConv <= ICC_VIRTUAL_THISCALL_OBJLAST_RETURNINMEM))
    {
    ...
    switch (callConv)
    {
        ...
        case ICC_VIRTUAL_THISCALL_OBJLAST_RETURNINMEM:
        {
            ...
            asFUNCTION_t* vftable = *(asFUNCTION_t**)obj;
            retQW =
                CallThisCallFunctionRetByRef(obj, args, paramSize << 2, vftable[FuncPtrToUInt(func) >> 2], retPointer);
        }
        break;
...
endcopy:
    ...
    asm __volatile__(
        ...
        // TODO: runtime optimize: The prologue/epilogue shouldn't be needed if the correct cfi directives are used
        // below
        "pushl %%ebp               \n"
        ...
        _S(CLEAR_FPU_STACK) "\n"
                            ...
                            // Need to align the stack pointer so that it is aligned to 16 bytes when making the
                            // function call. It is assumed that when entering this function, the stack pointer is
                            // already aligned, so we need to calculate how much we will put on the stack during this
                            // call.
                            "movl  4(%%ebx), %%eax  \n" // paramSize
