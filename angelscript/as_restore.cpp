            ...
            if (funcDef->IsShared())
            {
                for (asUINT n = 0; n < engine->funcDefs.GetLength(); n++)
                {
                    ...
                    if (f2->name == fdt->name && f2->nameSpace == fdt->nameSpace &&
                        f2->parentClass == fdt->parentClass && f2->funcdef->IsSignatureExceptNameEqual(funcDef))
                    {
    ...
    for (i = 0; i < count && !error; ++i)
    {
        ...
        if (func->IsShared() && len != module->m_scriptFunctions.GetLength())
        {
            ...
            for (asUINT n = 0; n < engine->scriptFunctions.GetLength() && !error; n++)
            {
                ...
                if (realFunc && realFunc != func && realFunc->IsShared() && realFunc->nameSpace == func->nameSpace &&
...
void asCReader::ReadUsedStringConstants()
{
    ...
    {
        ReadString(&str);
        usedStringConstants.PushLast(
            const_cast<void*>(engine->stringFactory->GetStringConstant(str.AddressOf(), (asUINT)str.GetLength())));
    }
}
                ...
                else
                {
                    for (asUINT i = 0; i < engine->scriptFunctions.GetLength(); i++)
                    {
                        ...
                        if (f == 0 || !f->IsShared() || func.objectType != f->objectType ||
                            func.funcType != f->funcType || func.nameSpace != f->nameSpace || !func.IsSignatureEqual(f))
                            continue;
            ...
            else
            {
                ...
                else if (func.name[0] == '$')
                {
                    ...
                    if (func.name == "$beh0" && func.objectType)
                    {
                        ...
                        {
                            // Look for the matching constructor inside the factory stubs generated for the template
                            // instance See asCCompiler::PerformFunctionCall
                    ...
                    {
                        // listFactory is used for both factory is global and returns a handle and constructor that is a
                        // method
                        asCObjectType* objType =
                            func.objectType ? func.objectType : CastToObjectType(func.returnType.GetTypeInfo());
                        if (objType)
                        {
                    ...
                    else if (func.name == "$dlgte")
                    {
                        ...
                        asCScriptFunction* f =
                            engine->registeredGlobalFuncs.GetFirst(engine->nameSpaces[0], DELEGATE_FACTORY);
                                           ...
                                           bool* isExternal)
{
            ...
            if (!(bits & 4))
            {
                ...
                if (bits & 8)
                {
                    ...
                    for (i = 0; i < length; ++i)
                    {
                        ...
                        if (option != asOBJ_INIT && option != asOBJ_UNINIT && option != asBLOCK_BEGIN &&
                            option != asBLOCK_END && option != asOBJ_VARDECL)
                        {
                            error = true;
                            func->DestroyHalfCreated();
                            return 0;
    ...
    if (func->objectType && func->objectType->module != module)
    {
        ...
        {
            // If the loaded function is a virtual function, then look for the identical virtual function in the methods
            // array
                    ...
                    if (func)
                    {
                        if (sharedExists)
                        {
                            ...
                            for (asUINT f = 0; f < ot->beh.constructors.GetLength(); f++)
                            {
                                ...
                                {
                                    // If the function is not the last, then the substitution has already occurred
                                    // before
...
int asCReader::AdjustGetOffset(int offset, asCScriptFunction* func, asDWORD programPos)
{
    ...
    // TODO: Can the asCScriptFunction::FindNextFunctionCalled be adapted so it can be reused here (support for
    // asBC_ALLOC, asBC_REFCPY and asBC_COPY)?
    ...
    for (asUINT n = programPos; n < func->scriptData->byteCode.GetLength();)
    {
        ...
        if (bc == asBC_CALL || bc == asBC_CALLSYS || bc == asBC_Thiscall1 || bc == asBC_CALLINTF || bc == asBC_ALLOC ||
            bc == asBC_CALLBND || bc == asBC_CallPtr)
        {
...
int asCWriter::Write()
{
    ...
    // TODO: Should be possible to skip saving the enum values. They are usually not needed after the script is compiled
    // anyway
    // TODO: Should be possible to skip saving the typedefs. They are usually not needed after the script is compiled
    // anyway
    // TODO: Should be possible to skip saving constants. They are usually not needed after the script is compiled
    // anyway
...
void asCWriter::WriteFunction(asCScriptFunction* func)
{
    ...
    if (func->funcType == asFUNC_SCRIPT)
    {
        ...
        // TODO: Store the type info/position in an intelligent way to avoid duplicating info in objVariablePos &
        // objVariableTypes.
        // TODO: Perhaps objVariablePos & objVariableTypes should be retired now that all variable types must be stored
        // anyway
...
void asCWriter::CalculateAdjustmentByPos(asCScriptFunction* func)
{
    ...
    for (n = 0; n < adjustments.GetLength(); n += 2)
    {
        ...
        // If more than one variable in different scopes occupy the same position on the stack they must have the same
        // size
    ...
    asUINT length =
        func->scriptData->byteCode.GetLength() + 1; // accomodate one more for invisible instructions, e.g. scope end
    asDWORD* bc = func->scriptData->byteCode.AddressOf();
...
int asCWriter::AdjustStackPosition(int pos)
{
    if (pos >= (int)adjustStackByPos.GetLength())
    {
        ...
        // The adjustByPos can also be empty if the function doesn't have any variables at all, but receive a handle by
        // parameter
...
void asCWriter::WriteByteCode(asCScriptFunction* func)
{
    ...
    while (length)
    {
                 ...
                 c == asBC_SetG4)      // PTR_DW_ARG
        {
            ...
            else
            {
                ...
                *(asPWORD*)(tmpBC + 1) = FindStringConstantIndex(*(void**)(tmpBC + 1)) << 1;
                 ...
                 c == asBC_JNS || c == asBC_JP || c == asBC_JNP) // The JMPP instruction doesn't need modification
        {
        ...
        //                 Some platforms may not use the IEEE 754 standard, in which case it is necessary to encode the
        //                 values
...
int asCWriter::FindObjectPropIndex(short offset, int typeId, asDWORD* bc)
{
    ...
    {
        // TODO: Composite: Perhaps it would be better to add metadata to the bytecode instruction to give the exact
        // property.
