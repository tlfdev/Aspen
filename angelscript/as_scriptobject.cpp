...
// TODO: Clean up: This function is almost identical to ScriptObjectFactory. Should make better use of the identical
// code.
...
void RegisterScriptObject(asCScriptEngine* engine)
{
    ...
    asASSERT(r >= 0);
    r = engine->RegisterBehaviourToObjectType(&engine->scriptTypeBehaviours, asBEHAVE_RELEASEREFS, "void f(int&in)",
                                              asFUNCTION(ScriptObject_ReleaseAllHandles_Generic), asCALL_GENERIC, 0);
    asASSERT(r >= 0);
#endif
}
...
asCScriptObject::asCScriptObject(asCObjectType* ot, bool doInitialize)
{
    ...
    {
        // When the object is created without initialization, all non-handle members must be allocated, but not
        // initialized
