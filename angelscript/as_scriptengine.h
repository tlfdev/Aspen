...
class asCScriptEngine : public asIScriptEngine
{
  ...
  public:
    ...
    virtual void GetGCStatistics(asUINT* currentSize, asUINT* totalDestroyed, asUINT* totalDetected, asUINT* newObjects,
  ...
  public:
    ...
    // TODO: memory savings: Since there can be only one property with the same name a simpler symbol table should be
    // used for global props
    ...
    //                 a funcdef becomes shared if all arguments and the return type are shared (or application
    //                 registered)
