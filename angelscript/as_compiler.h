...
struct asCExprContext
{
    ...
    bool isVoidExpression; // Set to true if the expression is an explicit 'void', e.g. used to ignore out parameters in
                           // func calls
    ...
    bool isAnonymousInitList;      // Set to true if the expression is an init list for which the type has not yet been
                                   // determined
};
...
class asCCompiler
{
  ...
  protected:
    ...
    void CompileInitAsCopy(asCDataType& type, int offset, asCExprContext* ctx, asCExprContext* arg, asCScriptNode* node,
                           bool derefDestination, EVarGlobOrMem isVarGlobOrMem = asVGM_VARIABLE);
