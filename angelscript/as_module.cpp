...
void asCModule::Discard()
{
    ...
    // Don't do this if the engine is already shutting down, as it will be done explicitly by the engine itself with
    // error reporting
    ...
    if (prop->GetInitFunc())
    {
        ...
        if (r >= 0)
        {
            ...
            if (r != asEXECUTION_FINISHED)
            {
                ...
                m_engine->WriteMessage(scriptSection ? scriptSection : "", row, col, asMSGTYPE_ERROR, msg.AddressOf());
                ...
                if (r == asEXECUTION_EXCEPTION)
                {
                    ...
                    m_engine->WriteMessage(scriptSection ? scriptSection : "", ctx->GetExceptionLineNumber(), 0,
...
bool asCModule::HasExternalReferences(bool shuttingDown)
{
    ...
    for (asUINT n = 0; n < m_scriptFunctions.GetLength(); n++)
    {
        ...
        {
            // If the func is shared and can be moved to another module then this is not a reason to keep the module
            // alive
    ...
    for (asUINT n = 0; n < m_classTypes.GetLength(); n++)
    {
        ...
        {
            // If the obj is shared and can be moved to another module then this is not a reason to keep the module
            // alive
    ...
    for (asUINT n = 0; n < m_funcDefs.GetLength(); n++)
    {
        ...
        if (func && func->externalRefCount.get())
        {
            // If the funcdef is shared and can be moved to another module then this is not a reason to keep the module
            // alive
                                 ...
                                 bool isGlobalFunction, asSFunctionTraits funcTraits, asSNameSpace* ns)
{
    ...
    // Verify that we are not assigning either the final or override specifier(s) if we are registering a non-member
    // function
...
const char* asCModule::GetImportedFunctionDeclaration(asUINT index) const
{
    ...
    asCString* tempString = &asCThreadManager::GetLocalData()->string;
    // TODO: Allow the application to decide if the parameter name should be included or not (requires change in the
    // interface)
