...
void RegisterScriptDictionary_Native(asIScriptEngine* engine)
{
...
#if AS_CAN_USE_CPP11
    // With C++11 it is possible to use asGetTypeTraits to automatically determine the correct flags that represents the
    // C++ class
    ...
    r = engine->RegisterObjectMethod(
        ...
        asMETHODPR(CScriptDictionary, operator[], (const dictKey_t&) const, const CScriptDictValue*), asCALL_THISCALL);
    assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod(
        ...
        asMETHODPR(CScriptDictionary, opForValue1, (const CScriptDictionary::CScriptDictIter&) const, const dictKey_t&),
        asCALL_THISCALL);
    assert(r >= 0);
