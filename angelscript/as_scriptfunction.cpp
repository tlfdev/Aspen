...
//	r = engine->RegisterMethodToObjectType(&engine->functionBehaviours, "bool opEquals(const int &in)",
//asMETHOD(asCScriptFunction,operator==), asCALL_THISCALL); asASSERT( r >= 0 );
#else
    r = engine->RegisterBehaviourToObjectType(&engine->functionBehaviours, asBEHAVE_ADDREF, "void f()",
    ...
    asASSERT(r >= 0);
    r = engine->RegisterBehaviourToObjectType(&engine->functionBehaviours, asBEHAVE_RELEASEREFS, "void f(int&in)",
                                              asFUNCTION(ScriptFunction_ReleaseAllHandles_Generic), asCALL_GENERIC, 0);
    asASSERT(r >= 0);
//	r = engine->RegisterMethodToObjectType(&engine->functionBehaviours, "bool opEquals(const int &in)",
//asFUNCTION(ScriptFunction_opEquals_Generic), asCALL_GENERIC); asASSERT( r >= 0 );
#endif
    ...
    asASSERT(r >= 0);
#else
    r = engine->RegisterGlobalFunction("void f(int &in, int &in)", asFUNCTION(ScriptFunction_CreateDelegate_Generic),
                                       asCALL_GENERIC);
    asASSERT(r >= 0);
#endif
...
void asCScriptFunction::MakeDelegate(asCScriptFunction* func, void* obj)
{
    ...
    // TODO: optimize: Do we really need to copy? Whenever requested the delegate can simply return the delegated
    // methods' info directly
...
int asCScriptFunction::ParseListPattern(asSListPatternNode*& target, const char* decl, asCScriptNode* listNodes)
{
    ...
    while (listNodes)
    {
        ...
        else if (listNodes->nodeType == snDataType)
        {
            ...
            // For list factory we get the object type from the return type, for list constructor we get it from the
            // object type directly
            dt = builder.CreateDataTypeFromNode(listNodes, &code, engine->defaultNamespace, false,
                                                objectType ? objectType : CastToObjectType(returnType.GetTypeInfo()));
                                               ...
                                               bool includeParamNames) const
{
    ...
    if (!(returnType.GetTokenType() == ttVoid && objectType &&
          (name == objectType->name || (name.GetLength() > 0 && name[0] == '~') || name == "$beh0" || name == "$beh2")))
    {
                    ...
                    // Need to increase the reference for each global variable
                    {
                        ...
                        if (!prop)
                        {
                            ...
                            if (r >= 0)
                            {
                                ...
                                // Get a new pointer (depending on the string factory implementation it may actually be
                                // the same)
