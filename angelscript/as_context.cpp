                                      ...
                                      asDWORD* _stackPointer, asDWORD* _stackIndex)
{
    ...
    if (_stackFramePointer)
        *_stackFramePointer = sfp; // TODO: Calculate stack frame pointer as delta from previous stack frame pointer (Or
                                   // perhaps it will always be the same as the stack pointer in previous function?)
    if (_currentFunction)
        *_currentFunction = currentFunction;
    if (_programPointer)
        *_programPointer =
            programPointer != 0 ? asUINT(programPointer - currentFunction->scriptData->byteCode.AddressOf()) : -1;
                                  ...
                                  asITypeInfo* objectType)
{
    ...
    else
    {
        ...
        if (tmp[0] != 0)
            return asERROR; // TODO: This is not really an error. It just means that the stackLevel doesn't represent a
                            // pushed state
        ...
        tmp[4] =
            (asPWORD)argumentsSize; // TODO: Calculate this from the initialFunction so it doesn't need to be serialized
                                      ...
                                      asDWORD _programPointer, asDWORD stackPointer, asDWORD stackIndex)
{
    ...
    // TODO: The arg _currentFunction is just used in debug mode to validate that it is the same that is already given
    // in m_currentFunction or on the call stack. Do we really need to take this argument?
...
int asCContext::Prepare(asIScriptFunction* func)
{
    ...
    // Reset state
    // Most of the time the previous state will be asEXECUTION_FINISHED, in which case the values are already
    // initialized
...
#ifdef WIP_16BYTE_ALIGN
        // Align the stack pointer. This is necessary as the m_stackBlockSize is not necessarily evenly divisable with
        // the max alignment
...
void asCContext::PrepareScriptFunction()
{
    ...
    // TODO: Need a fast way to iterate over this list (perhaps a pointer in variables to give index of next object var,
    // or perhaps just order the array with object types first)
    ...
    for (;;)
    {
		...
		*(int*)&m_regs.valueRegister = (*(int*)&m_regs.valueRegister > 0 ? 0 : VALUE_OF_BOOLEAN_TRUE);
            ...
            *(asPWORD*)l_sp = 0;
            ...
            INSTRUCTION(asBC_ClrVPtr)
                : // TODO: runtime optimize: Is this instruction really necessary?
                  //                         CallScriptFunction() can clear the null handles upon entry, just as is done
                  //                         for all other object variables
            ...
            *(asPWORD*)l_sp = (asPWORD)asBC_SWORDARG0(l_bc);
            ...
            INSTRUCTION(asBC_fTOu)
                : // We must cast to int first, because on some compilers the cast of a negative float value to uint
                  // result in 0
		...
		*(double*)(l_fp - asBC_SWORDARG0(l_bc)) = double(*(asQWORD*)(l_fp - asBC_SWORDARG0(l_bc)));
            ...
            *(asQWORD*)l_sp = *(asQWORD*)(l_fp - asBC_SWORDARG0(l_bc));
            ...
            INSTRUCTION(asBC_JLowZ) : if (*(asBYTE*)&m_regs.valueRegister == 0) l_bc += asBC_INTARG(l_bc) + 2;
            else l_bc += 2;
            NEXT_INSTRUCTION();

            INSTRUCTION(asBC_JLowNZ) : if (*(asBYTE*)&m_regs.valueRegister != 0) l_bc += asBC_INTARG(l_bc) + 2;
            else l_bc += 2;
            NEXT_INSTRUCTION();
            ...
            INSTRUCTION(asBC_POWi) :
            {
                bool isOverflow;
                *(int*)(l_fp - asBC_SWORDARG0(l_bc)) =
                    as_powi(*(int*)(l_fp - asBC_SWORDARG1(l_bc)), *(int*)(l_fp - asBC_SWORDARG2(l_bc)), isOverflow);
                if (isOverflow)
                {
                    // Need to move the values back to the context
                    m_regs.programPointer = l_bc;
            ...
            {
                bool isOverflow;
                *(asDWORD*)(l_fp - asBC_SWORDARG0(l_bc)) = as_powu(
                    *(asDWORD*)(l_fp - asBC_SWORDARG1(l_bc)), *(asDWORD*)(l_fp - asBC_SWORDARG2(l_bc)), isOverflow);
                if (isOverflow)
                {
                    // Need to move the values back to the context
                    m_regs.programPointer = l_bc;
            ...
            {
                bool isOverflow;
                *(asINT64*)(l_fp - asBC_SWORDARG0(l_bc)) = as_powi64(
                    *(asINT64*)(l_fp - asBC_SWORDARG1(l_bc)), *(asINT64*)(l_fp - asBC_SWORDARG2(l_bc)), isOverflow);
                if (isOverflow)
                {
                    // Need to move the values back to the context
                    m_regs.programPointer = l_bc;
            ...
            {
                bool isOverflow;
                *(asQWORD*)(l_fp - asBC_SWORDARG0(l_bc)) = as_powu64(
                    *(asQWORD*)(l_fp - asBC_SWORDARG1(l_bc)), *(asQWORD*)(l_fp - asBC_SWORDARG2(l_bc)), isOverflow);
                if (isOverflow)
                {
                    // Need to move the values back to the context
                    m_regs.programPointer = l_bc;
...
void asCContext::SetInternalException(const char* descr, bool allowCatch)
{
    ...
    if (m_currentFunction->scriptData)
    {
        m_exceptionLine = m_currentFunction->GetLineNumber(
            int(m_regs.programPointer - m_currentFunction->scriptData->byteCode.AddressOf()), &m_exceptionSectionIdx);
...
bool asCContext::IsVarInScope(asUINT varIndex, asUINT stackLevel)
{
    ...
    // Temporary variables aren't explicitly declared, they are just reserved slots available throughout the function
    // call. So we'll consider that the variable declaration is found at the very beginning
            ...
            for (--n; n >= 0; n--)
            {
                switch (func->scriptData->objVariableInfo[n].option)
                {
                    case asOBJ_UNINIT: // Object was destroyed
                    {
                        ...
                        // Which variable is this? Use IsVarInScope to get the correct variable in case there are
                        // multiple variables sharing the same offset
                        asUINT var = asUINT(-1);
...
void asCContext::CleanArgsOnStack()
{
    ...
    for (asUINT n = 0; n < func->parameterTypes.GetLength(); n++)
    {
        ...
        {
            // TODO: cleanup: This logic is repeated twice in CleanStackFrame too. Should create a common function to
            // share the code
...
bool asCContext::CleanStackFrame(bool catchException)
{
    ...
    if (!m_isStackMemoryNotAllocated && m_regs.programPointer)
    {
        ...
        for (asUINT n = 0; n < m_currentFunction->scriptData->variables.GetLength(); n++)
        {
            ...
            if (exceptionCaught)
            {
                // Find out where the variable was declared, and skip cleaning of those that were declared before the
                // try catch Multiple variables in different scopes may occupy the same slot on the stack so it is
                // necessary to search the entire list to determine which variable occupies the slot now.
                int skipClean = 0;
            ...
            if (m_currentFunction->scriptData->variables[n]->onHeap)
            {
                ...
                {
                    // Skip pointers with unknown types, as this is either a null pointer or just a reference that is
                    // not owned by function
                        ...
                        !m_currentFunction->scriptData->variables[n]->type.IsReference())
                    {
                        ...
                        else if (m_currentFunction->scriptData->variables[n]->type.GetTypeInfo()->flags & asOBJ_REF)
                        {
                            asSTypeBehaviour* beh =
                                &CastToObjectType(m_currentFunction->scriptData->variables[n]->type.GetTypeInfo())->beh;
                            asASSERT((m_currentFunction->scriptData->variables[n]->type.GetTypeInfo()->flags &
                                      asOBJ_NOCOUNT) ||
                                     beh->release);
                            if (beh->release)
                        ...
                        else
                        {
                            ...
                            if (beh->destruct)
                                m_engine->CallObjectMethod((void*)*(asPWORD*)&m_regs.stackFramePointer[-pos],
                                                           beh->destruct);
                            else if (m_currentFunction->scriptData->variables[n]->type.GetTypeInfo()->flags &
                                     asOBJ_LIST_PATTERN)
                                m_engine->DestroyList(
                                    (asBYTE*)*(asPWORD*)&m_regs.stackFramePointer[-pos],
                                    CastToObjectType(m_currentFunction->scriptData->variables[n]->type.GetTypeInfo()));
            ...
            else
            {
                ...
                if (liveObjects[n] > 0)
                {
                    asASSERT(m_currentFunction->scriptData->variables[n]->type.GetTypeInfo() &&
                             m_currentFunction->scriptData->variables[n]->type.GetTypeInfo()->GetFlags() & asOBJ_VALUE);
    ...
    // If the exception was caught then move the program position and stack pointer to the catch block then stop the
    // unwinding
    if (exceptionCaught)
    {
        m_regs.stackPointer =
                                  ...
                                  bool returnAddressOfUnitializedObjects)
{
        ...
        (pos <= 0))
    {
        ...
        // If dontDereference is true then the application wants the address of the reference, rather than the value it
        // refers to
                                          ...
                                          asDWORD*& p, asCScriptFunction*& realFunc)
{
    ...
    {
        // TODO: Instead of returning pointer, this should count number of instructions so that the deserialized program
        // pointer is 32/64bit agnostic
...
{
    // TODO: This function should find the correct stack block and then get the address within that stack block. It must
    // not be expected that the same initContextStackSize was used when the stack pointer was serialized
...
{
    // TODO: This function should determine actual offset from the lowest stack by unwinding the stack. This way when
    // deserializing it doesn't matter if the same block sizes are used or not
    asASSERT(v != 0);
    ...
    // Return the seriaized pointer as the offset in the lower 26 bits + the index of the stack block in the upper 6
    // bits
...
int asCContext::GetArgsOnStackCount(asUINT stackLevel)
{
    ...
    if (calledFunc)
        stackPos -= calledFunc->GetSpaceNeededForArguments() + (calledFunc->DoesReturnOnStack() ? AS_PTR_SIZE : 0) +
    ...
    while (stackPos > 0)
    {
        ...
        while (stackPos > 0)
        {
            if (++param < int(calledFunc->GetParamCount()))
            {
                ...
                if ((flags & asTM_INOUTREF) || (typeId & asTYPEID_MASK_OBJECT))
                {
                    // TODO: The value pushed on the stack here can be just the offset of the variable, not the actual
                    // pointer
                    stackPos -= AS_PTR_SIZE;
                }
                else if (typeId == asTYPEID_UINT64 || typeId == asTYPEID_INT64 || typeId == asTYPEID_DOUBLE)
