...
// - If a class method returns a reference, the caller must guarantee the object pointer stays alive until the function
// returns, and the reference is no longer going to be used
// - If a class method doesn't return a reference, it must guarantee by itself that the this pointer stays alive during
// the function call. If no outside access is made, then the function is guaranteed to stay alive and nothing needs to
// be done
// - The object pointer is always passed as the first argument, position 0
// - If the function returns a value type the caller must reserve the memory for this and pass the pointer as the first
// argument after the object pointer
//
...
//
//       The compiler shouldn't use the asCDataType::IsReference. The datatype should always be stored as
//       non-references. Instead the compiler should keep track of references in TypeInfo, where it should also state
//       how the reference is currently stored, i.e. in variable, in register, on stack, etc.
...
void asCCompiler::FinalizeFunction()
{
    ...
    // Extract the script section indexes too if there are any entries that are different from the function's script
    // section
        ...
        if (declNode)
        {
            ...
            if (prop->type.IsObject() && !prop->type.IsObjectHandle() &&
                (((prop->type.GetTypeInfo()->flags & asOBJ_REF) && prop->type.GetBehaviour()->factory == 0) ||
                 ((prop->type.GetTypeInfo()->flags & asOBJ_VALUE) && prop->type.GetBehaviour()->construct == 0 &&
                                 ...
                                 asCScriptFunction* in_outFunc, sClassDeclaration* in_classDecl)
{
    ...
    // TODO: optimize: For large functions, the parsing of the statement block can take a long time. Presumably because
    // a lot of memory needs to be allocated
    asCParser parser(builder);
    ...
    if (m_isConstructor)
    {
        // From 2.38.0 it was implemented the ability to explicitly initialize the members within the body, and with
        // this it was changed so that any member not explicitly initialized will be automatically initialized in the
        // beginning even if the base class' constructor is explicitly called in the body.
        ...
        else
        {
            ...
            if (outFunc->objectType->derivedFrom)
            {
                ...
                else
                {
                    ...
                    // The members that are explicitly initialized will be initialized after the call to base class'
                    // constructor
                    CompileMemberInitialization(&byteCode, true);
                }
            }
                                 ...
                                 int refType, bool isMakingCopy)
{
    ...
    {
        // The function is expecting a var type. If the argument is a function name, we must now decide which function
        // it is
    ...
    if (isFunction && dt.IsReference())
    {
        ...
        if (refType == asTM_INREF)
        {
            ...
            else if (ctx->type.dataType.IsNullHandle())
            {
                ...
                // Don't allow null handle to be converted to an object type of ASHANDLE here, that would require more
                // logic to call the constructor (which should be handled in ImplicitConversion)
            ...
            else
            {
                ...
                if (!isMakingCopy)
                {
                    ...
                    if (dt.IsObjectHandle() && !ctx->type.dataType.IsObjectHandle() && !ctx->IsAnonymousInitList() &&
                        ctx->type.dataType.SupportHandles())
                    {
        ...
        else if (refType == asTM_INOUTREF)
        {
            ...
            if (!engine->ep.allowUnsafeReferences && !ctx->type.isVariable &&
                ...
                ((ctx->type.dataType.GetBehaviour()->addref && ctx->type.dataType.GetBehaviour()->release) ||
                 (ctx->type.dataType.GetTypeInfo()->flags & asOBJ_NOCOUNT) || ctx->type.dataType.IsFuncdef()))
            {
...
void asCCompiler::MoveArgsToStack(int funcId, asCByteCode* bc, asCArray<asCExprContext*>& args, bool addOneToOffset)
{
    ...
    for (asUINT n = 0; n < args.GetLength(); n++)
    {
        ...
        {
            if ((descr->parameterTypes[realParamIdx].IsObject() || descr->parameterTypes[realParamIdx].IsFuncdef()) &&
                !descr->parameterTypes[realParamIdx].IsObjectHandle())
            {
                if (descr->inOutFlags[realParamIdx] != asTM_INOUTREF && !args[n]->type.isRefSafe)
                {
    #ifdef AS_DEBUG
                    // This assert is inside AS_DEBUG because of the variable makingCopy which is only defined in debug
                    // mode
                                            ...
                                            asCObjectType* objectType, asCArray<asSNamedArgument>* namedArgs)
{
    ...
    for (int n = (int)func->parameterTypes.GetLength() - 1; n >= explicitArgs; n--)
    {
        ...
        asCParser parser(builder);
        asCScriptCode* code =
            builder->FindOrAddCode("default arg", func->defaultArgs[n]->AddressOf(), func->defaultArgs[n]->GetLength());
                                        ...
                                        EVarGlobOrMem isVarGlobOrMem, asCExprContext* preCompiled)
{
    ...
    else if (node && node->nodeType == snAssignment)
    {
        ...
        return CompileInitializationWithAssignment(bc, type, errNode, offset, constantValue, isVarGlobOrMem, node,
                                                   expr);
    }
    else
    {
                                                      ...
                                                      asCScriptNode* rnode, asCExprContext* rexpr)
{
    ...
    // TODO: For value types, stored inline the rexpr would have to be compiled already to initialize the value so the
        ...
        else
        {
            ...
            if ((lexpr.type.dataType.IsObject() || lexpr.type.dataType.IsFuncdef()) &&
                (!lexpr.type.isExplicitHandle ||
                 (lexpr.type.dataType.GetTypeInfo() && (lexpr.type.dataType.GetTypeInfo()->flags & asOBJ_ASHANDLE))))
            {
                                        ...
                                        int& elementsInSubList)
{
    if (patternNode->type == asLPT_START)
    {
        ...
        {
            // Check for missing value here, else the error reporting will not have a source position to report the
            // error for
    ...
    else if (patternNode->type == asLPT_REPEAT || patternNode->type == asLPT_REPEAT_SAME)
    {
        ...
        if (countElements == 0)
        {
            // Skip the sub pattern that was expected to be repeated, otherwise the caller will try to match these when
            // we return
            patternNode = nextNode;
        ...
        if (valueNode->nodeType == snAssignment || valueNode->nodeType == snInitList)
        {
            ...
            if (valueNode->nodeType == snAssignment)
            {
                ...
                if (dt.GetTokenType() == ttQuestion)
                {
                    ...
                    // When value assignment for reference types us disabled, make sure all ref types are passed in as
                    // handles
            ...
            else if (valueNode->nodeType == snInitList)
            {
                if (dt.GetTokenType() == ttQuestion)
                {
                    // Can't use init lists with var type as it is not possible to determine what type should be
                    // allocated
                    asCString str;
        ...
        else
        {
            ...
            else
            {
                ...
                else if (dt.GetTypeInfo() && dt.GetTypeInfo()->flags & asOBJ_VALUE)
                {
                    ...
                    {
                        // Values on the list must be aligned to 32bit boundaries, except if the type is smaller than
                        // 32bit.
                        if (bufferSize & 0x3)
                            bufferSize += 4 - (bufferSize & 0x3);
...
void asCCompiler::CompileSwitchStatement(asCScriptNode* snode, bool* hasReturn, asCByteCode* bc)
{
    *hasReturn = false;
    ...
    // If a case doesn't have a return, but falls through to a case that does return then the case can be said to have a
    // return too
    *hasReturn = true;
...
void asCCompiler::CompileIfStatement(asCScriptNode* inode, bool* hasReturn, asCByteCode* bc)
{
    ...
    if (m_isConstructor && initializedProperties1 != initializedProperties2)
    {
        ...
        // Use the max property access count so a member accessed in only one of the conditions will be considered as
        // well
...
void asCCompiler::CompileForEachStatement(asCScriptNode* node, asCByteCode* bc)
{
    ...
    while (rangeNode->nodeType != snAssignment)
    {
        ...
        asCDataType itemDt = builder->CreateDataTypeFromNode(rangeNode, script, outFunc->nameSpace, false,
                                                             outFunc->objectType, true, 0, 0, &m_namespaceVisibility);
    ...
    CompileInitializationWithAssignment(&rangeExpr.bc, rangeDt, rangeNode, rangeOffset, 0, asVGM_VARIABLE, rangeNode,
                                        &rangeExpr);
    ...
    if (iterTid)
    {
        ...
        for (asUINT i = 0; i < funcs.GetLength(); i++)
        {
            ...
            asDWORD flags;
            int paramTid;
            if (f->GetParamCount() == 1 && f->GetReturnTypeId(&flags) == asTYPEID_BOOL && !(flags & asTM_INOUTREF) &&
    ...
    if (iterTid)
    {
        ...
        for (asUINT i = 0; i < funcs.GetLength(); i++)
        {
            asIScriptFunction* f = engine->scriptFunctions[funcs[i]];
            int paramTid;
            if (f->GetParamCount() == 1 && f->GetReturnTypeId() == iterTid && f->GetParam(0, &paramTid) >= 0 &&
                paramTid == iterTid)
            {
    ...
    if (iterTid)
    {
        ...
        if (itemCount > 1 || opForValueNIds.GetLength() == 0)
        {
            for (asUINT i = 0; i < itemCount; ++i)
            {
                ...
                if (itemCount == 1)
                    builder->GetObjectMethodDescriptions(VALUE_NAME, CastToObjectType(rangeTypeInfo), funcs,
                                                         isConstRange);
                if (funcs.GetLength() == 0)
                {
                    methodName.Format("%s%d", VALUE_NAME, (int)i);
                    builder->GetObjectMethodDescriptions(methodName.AddressOf(), CastToObjectType(rangeTypeInfo), funcs,
                                                         isConstRange);
                }
    ...
    asCExprContext opForBeginExpr(engine);
    {
        ...
        asCArray<asCExprContext*> args;
        int r = MakeFunctionCall(&opForBeginExpr, opForBeginId, CastToObjectType(rangeExpr.type.dataType.GetTypeInfo()),
                                 args, node->firstChild);
        UNUSED_VAR(r);
        asASSERT(r >= 0);
    }
    CompileInitializationWithAssignment(&initBC, iterDt, node->firstChild, iterOffset, NULL, asVGM_VARIABLE, rangeNode,
                                        &opForBeginExpr);
    ...
    asCExprContext opForEndExpr(engine);
    {
        ...
        int r = MakeFunctionCall(&opForEndExpr, opForEndId, CastToObjectType(rangeExpr.type.dataType.GetTypeInfo()),
                                 args, node);
        UNUSED_VAR(r);
        asASSERT(r >= 0);
    ...
    asCExprContext next(engine);
    {
        ...
        int r = MakeFunctionCall(&opForNextExpr, opForNextId, CastToObjectType(rangeExpr.type.dataType.GetTypeInfo()),
                                 args, node);
        UNUSED_VAR(r);
        asASSERT(r >= 0);
    ...
    for (asUINT i = 0; i < itemCount; ++i)
    {
        ...
        arg.exprNode = node;
        args.PushLast(&arg);
        int r = MakeFunctionCall(&opForValueNExpr, opForValueNIds[i],
    ...
    if (rnode->firstChild)
    {
        ...
        if (v->type.IsReference())
        {
            ...
            if (!(v->type.IsEqualExceptConst(expr.type.dataType) ||
                  ((expr.type.dataType.IsObject() || expr.type.dataType.IsFuncdef()) &&
                   !expr.type.dataType.IsObjectHandle() && v->type.IsEqualExceptRefAndConst(expr.type.dataType))) ||
        ...
        else // if( !v->type.IsReference() )
        {
            ...
            else if (v->type.IsObject() || v->type.IsFuncdef())
            {
                ...
                if (outFunc->DoesReturnOnStack())
                {
                    // TODO: runtime optimize: If the return type has a constructor that takes the type of the
                    // expression,
                    //                         it should be called directly instead of first converting the expression
                    ...
                    // If this return statement is within a branch, there might be more statements after this. To avoid
                    // having the exception handler think the object is live in the statements between the return and
                    // the end of the function we need to add a marker
...
void asCCompiler::PrintMatchingFuncs(asCArray<int>& funcs, asCScriptNode* node, asCObjectType* inType)
{
    ...
    for (unsigned int n = 0; n < funcs.GetLength(); n++)
    {
        ...
        {
            // Check for funcdefs in the arguments that may have been generated by the template instance, so these can
            // be shown to user
                                       ...
                                       asCExprContext* lvalueExpr)
{
    ...
    if (lvalue->IsPrimitive())
    {
        ...
        {
            asCString str;
            str.Format(TXT_CANT_IMPLICITLY_CONVERT_s_TO_s, rctx->type.dataType.Format(outFunc->nameSpace).AddressOf(),
    ...
    else if (!lvalue->isExplicitHandle)
    {
        ...
        *lvalue = ctx.type;
        ...
        else
        {
            ...
            if (lvalue->dataType.GetSizeInMemoryDWords() == 0 || !(lvalue->dataType.GetTypeInfo()->flags & asOBJ_POD))
            {
                asCString msg;
    ...
    if (ops.GetLength() == 1)
    {
        ...
        if (generateCode)
        {
                ...
                !(ctx->type.dataType.GetTypeInfo()->flags & asOBJ_NOHANDLE))
            {
                ...
                // TODO: runtime optimize: ClrVPtr is not necessary, because the VM should initialize the variable to
                // null anyway (it is currently not done for null pointers though)
        ...
        if (ops.GetLength() == 1)
        {
            ...
            if (generateCode)
            {
                    ...
                    !(ctx->type.dataType.GetTypeInfo()->flags & asOBJ_NOHANDLE))
                {
                    ...
                    // TODO: runtime optimize: should have immediate comparison for null pointer
                    int offset = AllocateVariable(asCDataType::CreateNullHandle(), true);
                    // TODO: runtime optimize: ClrVPtr is not necessary, because the VM should initialize the variable
                    // to null anyway (it is currently not done for null pointers though)
                                                     ...
                                                     asCScriptNode* node, EImplicitConv convType, bool generateCode)
{
    ...
    {
        if (((to.IsIntegerType() && !to.IsEnumType()) || to.IsUnsignedType() || to.IsFloatType() || to.IsDoubleType() ||
             (to.IsEnumType() && convType == asIC_EXPLICIT_VAL_CAST)) &&
            (ctx->type.dataType.IsIntegerType() || ctx->type.dataType.IsUnsignedType() ||
             ctx->type.dataType.IsFloatType() || ctx->type.dataType.IsDoubleType()))
        {
                                             ...
                                             EImplicitConv /*convType*/, bool generateCode)
{
    ...
    {
        // There will be one node for each parameter. There will be 0, 1, or 2 children in the node with datatype and/or
        // name
        if (argNode->nodeType == snUndefined)
        {
            ...
            {
                asCDataType dt =
                    builder->CreateDataTypeFromNode(typeNode, script, outFunc->nameSpace, false, outFunc->objectType,
                                            ...
                                            EImplicitConv convType, bool generateCode)
{
    ...
    if (to.GetTypeInfo() != ctx->type.dataType.GetTypeInfo())
    {
        ...
        else
        {
            ...
            {
                // If no opConv/opImplConv methods were found on the object, then try to find a conversion constructor
                // on the target type
                                               ...
                                               EImplicitConv convType, bool generateCode, bool allowObjectConstruct)
{
    ...
    if (to.IsObjectHandle())
    {
        ...
        {
            // String literals can be implicitly converted to temporary local variables in order to pass them to
            // functions expecting non-const
            ...
            else // if( !ctx->type.dataType.IsReference() )
            {
                ...
                else
                {
                    ...
                    if ((ctx->type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE) &&
                        (ctx->type.isVariable || ctx->type.isTemporary) && !IsVariableOnHeap(ctx->type.stackOffset))
                    {
                                                  ...
                                                  bool allowObjectConstruct)
{
    ...
    {
        // For value types the object must have a constructor that takes a single primitive argument either by value or
        // as input reference
        for (asUINT n = 0; n < objType->beh.constructors.GetLength(); n++)
        {
            ...
            if (func->parameterTypes.GetLength() == 1 && func->parameterTypes[0].IsPrimitive() &&
                !(func->inOutFlags[0] & asTM_OUTREF) && (isExplicit == asIC_EXPLICIT_VAL_CAST || !func->IsExplicit()))
            {
                funcs.PushLast(func->id);
            }
        }
    ...
    {
        // For ref types the object must have a factory that takes a single primitive argument either by value or as
        // input reference
                              ...
                              asCScriptNode* rexpr, eTokenType op, asCScriptNode* opNode)
{
        ...
        !(lctx->type.dataType.IsObjectHandle() && !lctx->type.isExplicitHandle))
    {
        ...
        if (lctx->property_set && lctx->type.isExplicitHandle)
        {
            ...
            asCArray<asCDataType>& parameterTypes = builder->GetFunctionDescription(lctx->property_set)->parameterTypes;
    ...
    if (engine->ep.memberInitMode == 1 && m_isConstructor)
    {
        ...
        // TODO: Should probably have a boolean on asCExprContext to indicate that it is a property, and have it set in
        // CompilerVariableAccess, and then cleared if any pre/post operation is done it
        asCByteInstruction* instr = lctx->bc.GetFirstInstr();
        if (instr && instr->op == asBC_PSF && instr->wArg[0] == 0 && instr->next && instr->next->op == asBC_RDSPtr &&
            ...
            ((instr->next->next->next && instr->next->next->next->op == asBC_RDSPtr &&
             ...
             (instr->next->next->next && instr->next->next->next->op == asBC_RDSPtr && instr->next->next->next->next &&
              instr->next->next->next->next->op == asBC_PopRPtr && instr->next->next->next->next->next == 0) ||
              ...
              instr->next->next->next->next == 0)))
        {
            ...
            if (!(prop && prop->isInherited))
            {
                ...
                // TODO: Need to keep track of the code path, as the initialization may have been in a different if-else
                // scope
                if (prop && m_initializedProperties.IndexOf(prop) < 0)
                {
                    ...
                    asQWORD constantValue;
                    CompileInitializationWithAssignment(&ctx->bc, prop->type, opNode, offset, &constantValue,
                                                        asVGM_MEMBER, rexpr, rctx);
    ...
    else // if( lctx->type.dataType.IsObject() )
    {
        ...
        // TODO: It must be possible to register specific types that should be allowed to be treated as lvalue even if
        // they are temporary
        //       These can for example be used by types that are really proxies for more complex types. When it is
        //       possible to do so I can reintroduce this check to prevent accidental bugs with assigning value to
        //       temporary objects
        ...
        bool simpleExpr =
            (lctx->type.dataType.GetTypeInfo() && lctx->type.dataType.GetTypeInfo()->GetFlags() & asOBJ_VALUE) &&
        ...
        else
        {
            ...
            if (rctx->type.dataType.IsReference() &&
                (!(rctx->type.isVariable || rctx->type.isTemporary) || IsVariableOnHeap(rctx->type.stackOffset)))
        ...
        if (!simpleExpr || needConversion)
        {
            if (!rctx->type.isRefSafe && (rctx->type.isVariable || rctx->type.isTemporary))
            {
                if (!IsVariableOnHeap(rctx->type.stackOffset))
                    // TODO: runtime optimize: Actually the reference can be pushed on the stack directly
                    //                         as the value allocated on the stack is guaranteed to be safe.
                    //                         The bytecode optimizer should be able to determine this and optimize away
                    //                         the VAR + GETREF
    ...
    if (cexpr->next)
    {
        ...
        if (lr >= 0 && rr >= 0)
        {
            ...
            else if (le.type.IsNullConstant() && re.type.IsNullConstant())
            {
                ...
                // TODO: Other expressions where both results are identical literal constants can probably also be
                // handled this way
            ...
            else
            {
                    ...
                    le.type.dataType == re.type.dataType)
                {
                    ...
                    // Must remember if the reference was to a local variable, since it must not be allowed to be
                    // returned
                ...
                else
                {
                    ...
                    // TODO: copy: Use copy constructor if available. See PrepareTemporaryVariable() or
                    // CompileInitAsCopy()
                    ...
                    // Process any deferred arguments in the expressions as these must not survive until after the
                    // condition returns
                    ProcessDeferredParams(ctx);
...
int asCCompiler::CompileExpressionTerm(asCScriptNode* node, asCExprContext* ctx)
{
    ...
    if (node->firstChild)
    {
        if (node->firstChild->nodeType == snDataType)
        {
            ...
            asCDataType dt = builder->CreateDataTypeFromNode(node->firstChild, script, outFunc->nameSpace, false, 0,
                                                             true, 0, 0, &m_namespaceVisibility);
...
// The purpose of this function is to find the entity that matches the symbol name respecting the scope and visibility
// hierarchy The 'outResult' will be used to return info on what was identified, but no code will be produced by this
// function input:
//  name         = the name of the symbol to look for
//  scope        = explicit scope informed
//  objType      = used to look for symbols within object type (e.g. when compiling post op), in this case no local or
//  global symbols will be looked up
// returns:
//  SL_NOMATCH          = no matching symbol
//  SL_LOCALCONST       = local constant
//  SL_LOCALVAR         = local variable
//  SL_THISPTR          = this pointer
//  SL_CLASSPROPACCESS  = class property accessor, lookupResult->dataType holds the object type in which the member was
//  found SL_CLASSPROP        = class property, lookupResult->dataType holds the object type in which the member was
//  found SL_CLASSMETHOD      = class method, lookupResult->dataType holds the object type in which the member was found
//  SL_CLASSTYPE        = class child type, lookupResult->dataType holds the object type in which the member was found
//  SL_GLOBALPROPACCESS = global property accessor, lookupResult->symbolNamespace holds the namespace where the symbol
//  was identified SL_GLOBALCONST      = global constant, lookupResult->symbolNamespace holds the namespace where the
//  symbol was identified SL_GLOBALVAR        = global variable, lookupResult->symbolNamespace holds the namespace where
//  the symbol was identified SL_GLOBALFUNC       = global function, lookupResult->symbolNamespace holds the namespace
//  where the symbol was identified SL_GLOBALTYPE       = type, lookupResult->dataType holds the type SL_ENUMVAL = enum
//  value, lookupResult->dataType holds the enum type, unless ambigious. lookupResult->symbolNamespace holds the
//  namespace where the symbol was identified SL_ERROR            = error
                                                  ...
                                                  asCExprContext* outResult, asCScriptNode* errNode)
{
    ...
    while (!objType && currNamespace)
    {
        if (!visitedNamespaces.Exists(currNamespace))
        {
            ...
            // If the scope contains ::identifier, then use the last identifier as the class name and the rest of it as
            // the namespace
            ...
            if (ns)
            {
                ...
                if (resultSymbolType != SL_GLOBALPROPACCESS)
                {
                    ...
                    asQWORD constantValue = 0;
                    asCGlobalProperty* prop = builder->GetGlobalProperty(name.AddressOf(), ns, &isCompiled,
            ...
            // Is it an enum value? Only do this if a symbol hasn't already been found, since it would hide the enum
            // value
        ...
        if (symbolType == SL_GLOBALCONST || symbolType == SL_GLOBALVAR)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ((ctx->type.dataType.GetTypeInfo()->flags & asOBJ_VALUE) || ctx->type.dataType.IsObjectHandle())
                    {
                        ctx->type.dataType.MakeReference(true);
                    }
                    else
                    {
                        ...
                        // It's necessary to dereference the pointer so the pointer on the stack will point to the
                        // actual object
                        ctx->bc.Instr(asBC_RDSPtr);
                    }
                }
...
int asCCompiler::CompileExpressionValue(asCScriptNode* node, asCExprContext* ctx)
{
    ...
    else if (vnode->nodeType == snConstant)
    {
        ...
        else if (vnode->tokenType == ttFloatConstant)
        {
            ...
            // Don't check this if we have double as float, because then the whole token would be scanned (i.e. no f
            // suffix)
...
void asCCompiler::AfterFunctionCall(int funcID, asCArray<asCExprContext*>& args, asCExprContext* ctx, bool deferAll)
{
    ...
    for (; n >= 0; n--)
    {
        ...
        // All &out arguments must be deferred, except if the argument is clean, in which case the actual reference was
        // passed in to the function If deferAll is set all objects passed by reference or handle must be deferred
        if ((descr->parameterTypes[realParamIdx].IsReference() && (descr->inOutFlags[realParamIdx] & asTM_OUTREF) &&
    ...
    for (asUINT n = 0; n < ctx->deferredParams.GetLength(); n++)
    {
        ...
        else if (outParam.argInOutFlags == asTM_OUTREF)
        {
            ...
            else
            {
                ...
                // Give an error, except if the argument is void, null or 0 which indicate the argument is explicitly to
                // be ignored
...
int asCCompiler::CompileConstructCall(asCScriptNode* node, asCExprContext* ctx)
{
    ...
    asCDataType dt;
    dt = builder->CreateDataTypeFromNode(node->firstChild, script, outFunc->nameSpace, false, outFunc->objectType, true,
                                         0, 0, &m_namespaceVisibility);
    if (dt.IsPrimitive())
    {
...
int asCCompiler::InstantiateTemplateFunctions(asCArray<int>& funcs, asCScriptNode* types)
{
    ...
    for (asUINT i = 0; i < funcs.GetLength(); i++)
    {
        ...
        // TODO: If types for template instance has been given in the node, and no matching template function exists
        // then an error must be given
        if (numTypes == 0)
            continue;
        ...
        for (asUINT j = 0; j < numTypes; j++)
        {
            ...
            dataTypes.PushLast(builder->CreateDataTypeFromNode(types, script, func->nameSpace, func->objectType, 0,
                                                               true, 0, 0, &m_namespaceVisibility));
            types = types->next;
        }
                                     ...
                                     bool objIsConst, const asCString& scope)
{
        ...
        symbolType == SL_GLOBALCONST || symbolType == SL_GLOBALVAR || symbolType == SL_ENUMVAL)
    {
        ...
        {
            // Keep information about temporary variables as deferred expression so it can be properly cleaned up after
            // the call
            if (ctx->type.isTemporary)
            {
                asASSERT(objectType);
    ...
    if (symbolType == SL_CLASSTYPE || symbolType == SL_GLOBALTYPE)
    {
        bool isValid = false;
        asCDataType dt =
            builder->CreateDataTypeFromNode(node->firstChild, script, outFunc->nameSpace, false, outFunc->objectType,
                                            false, &isValid, 0, &m_namespaceVisibility);
        if (isValid)
    ...
    if (CompileArgumentList(node->lastChild, args, namedArgs) >= 0)
    {
        // Special case: Allow calling func(void) with an expression that evaluates to no datatype, but isn't exactly
        // 'void'
        ...
        else
        {
            ...
            if (r == asSUCCESS)
            {
                ...
                if (func->funcType == asFUNC_FUNCDEF)
                {
                    ...
                    // The function call will be made directly from the local variable so the function pointer shouldn't
                    // be on the stack
...
int asCCompiler::CompileExpressionPreOp(asCScriptNode* node, asCExprContext* ctx)
{
             ...
             ctx->type.dataType.IsObject())
    {
        ...
        if (opName)
        {
            ...
            // TODO: If the value isn't const, then first try to find the non const method, and if not found try to find
            // the const method
                                      ...
                                      asCScriptNode* node, asSNameSpace* ns, bool isThisAccess)
{
    ...
    if (ctx->type.dataType.IsObject())
    {
        ...
        // property accessors have been disabled by the application
        if (!(ctx->type.dataType.GetTypeInfo()->flags & asOBJ_SCRIPT_OBJECT) || engine->ep.propertyAccessorMode >= 2)
        {
            ...
            for (asUINT n = 0; n < ot->methods.GetLength(); n++)
            {
                ...
                // TODO: getset: If the parameter is a reference, it must not be an out reference. Should we allow inout
                // ref?
        ...
        if (node->firstChild->nodeType == snIdentifier)
        {
            ...
            if (ctx->type.dataType.IsObject())
            {
                ...
                if (prop)
                {
                    ...
                    // This must always be done even if the offset is 0 because the asCWriter needs the meta data in
                    // ADDSi to identify the composite property
        ...
        if (CompileArgumentList(node->firstChild, args, namedArgs) >= 0)
        {
            ...
            if (lookForProperty && isOK)
            {
                ...
                else
                {
                    ...
                    // Check for accessors methods for the opIndex, either as get/set_opIndex or as get/set with the
                    // property name
            ...
            else
            {
                ...
                if (r == asSUCCESS)
                {
                    ...
                    if (ctx->type.dataType.IsFuncdef())
                    {
                        ...
                        // Remove the reference from the stack as the asBC_CALLPTR instruction takes the variable as
                        // argument
                        ctx->bc.Instr(asBC_PopPtr);
                    }
                               ...
                               bool allowObjectConstruct)
{
    ...
    // The cost for this conversion is not used, as we still want to use the same order of prioritization as for input
    // arguments
    ...
    // Don't allow the implicit conversion to make a copy in case the argument is expecting a reference to the true
    // value
                                                ...
                                                bool leftToRight, asCExprContext* ctx, bool isHandle, eTokenType token)
{
    ...
    {
        // TODO: Should evaluate which of the two have the best match. If both have equal match, the first version
        // should be used Find the matching opEquals method
        ...
        token == ttGreaterThan || token == ttGreaterThanOrEqual)
    {
        ...
        // TODO: Should evaluate which of the two have the best match. If both have equal match, the first version
        // should be used Find the matching opCmp method
    ...
    {
        // TODO: Should evaluate which of the two have the best match. If both have equal match, the first version
        // should be used Find the matching operator method
...
void asCCompiler::ConvertToVariable(asCExprContext* ctx)
{
    ...
    int offset;
    if (!ctx->type.isVariable &&
        (ctx->type.dataType.IsObjectHandle() || (ctx->type.dataType.IsObject() && ctx->type.dataType.SupportHandles())))
    {
    ...
    else
    {
        ...
        if (lctx->type.dataType.IsIntegerType() || lctx->type.dataType.IsUnsignedType())
        {
            if (lctx->type.dataType.GetSizeInMemoryDWords() == 1)
            {
                ...
                // If the right value is greater than the left value in a minus operation, then we need to convert the
                // type to int
            ...
            else
            {
                ...
                else if (op == ttSlash)
                {
                    ...
                    if (rctx->type.GetConstantQW() == 0 ||
                        (rctx->type.GetConstantQW() == asQWORD(-1) && lctx->type.GetConstantQW() == (asQWORD(1) << 63)))
                        v = 0;
                    else if (lctx->type.dataType.IsIntegerType())
                        v = asINT64(lctx->type.GetConstantQW()) / asINT64(rctx->type.GetConstantQW());
                    else
                                           ...
                                           asCExprContext* ctx, eTokenType opToken)
{
    ...
    if ((opToken == ttEqual || opToken == ttNotEqual) &&
        ((!(lctx->type.isExplicitHandle || lctx->type.IsNullConstant()) &&
          !(lctx->type.dataType.GetTypeInfo() && (lctx->type.dataType.GetTypeInfo()->flags & asOBJ_IMPLICIT_HANDLE))) ||
         (!(rctx->type.isExplicitHandle || rctx->type.IsNullConstant()) &&
          !(rctx->type.dataType.GetTypeInfo() && (rctx->type.dataType.GetTypeInfo()->flags & asOBJ_IMPLICIT_HANDLE)))))
    {
                                      ...
                                      int varOffset, int funcPtrVar)
{
    ...
    {
        // TODO: runtime optimize: Avoid this for global variables, by storing a reference to the global variable once
        // in a
        //                         local variable and then refer to the same for each call. An alias for the global
        //                         variable should be stored in the variable scope so that the compiler can find it. For
        //                         loops and scopes that will always be executed, i.e. non-if scopes the alias should be
        //                         stored in the higher scope to increase the probability of re-use.
    ...
    else
    {
        ...
        // Clean up arguments
        // If application is using unsafe references, then don't clean up arguments yet because
        // the returned value might represent a reference to one of the arguments, e.g. an integer might in truth be a
        // pointer
        if (args)
            AfterFunctionCall(funcId, *args, ctx, engine->ep.allowUnsafeReferences ? true : false);
