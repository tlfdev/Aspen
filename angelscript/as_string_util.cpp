...
// If base is 0 the string should be prefixed by 0x, 0d, 0o, or 0b to allow the function to automatically determine the
// radix
        ...
        if (base)
        {
            for (int nbr; (nbr = asCharToNbr(*end, base)) >= 0; end++)
            {
                if (overflow && ((res > QWORD_MAX / base) ||
                                 ((asUINT(nbr) > (QWORD_MAX - (QWORD_MAX / base) * base)) && res == QWORD_MAX / base)))
                    *overflow = true;
