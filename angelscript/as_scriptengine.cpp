...
asCScriptEngine::asCScriptEngine()
{
    ...
    // Engine properties
    {
        ...
        ep.genericCallMode = 1; // 0 = old (pre 2.33.0) behavior where generic ignored auto handles, 1 = treat handles
                                // like in native call
        ...
        ep.memberInitMode = 1; // 0 = pre 2.38.0, members with init expr in declaration are initialized after super(), 1
                               // = all members initialized in beginning, except if explicitly initialized in body
        ep.boolConversionMode = 0; // 0 = only do use opImplConv for registered value type, 1 = use also opConv in
                                   // contextual conversion even for reference types
...
int asCScriptEngine::ShutDownAndRelease()
{
    ...
    // Clear the context callbacks. If new context's are needed for the clean-up the engine will take care of this
    // itself. Context callbacks are normally used for pooling contexts, and if we allow new contexts to be created
    // without being immediately destroyed afterwards it means the engine's refcount will increase. This is turn may
    // cause memory access violations later on when the pool releases its contexts.
...
asSNameSpace* asCScriptEngine::AddNameSpace(const char* name)
{
    ...
    // TODO: thread-safety: This can potentially be called from multiple threads so it must be protected with critical
    // section
...
int asCScriptEngine::RegisterObjectType(const char* name, int byteSize, asQWORD flags)
{
    ...
    isPrepared = false; // TODO: Only set this after the validations have been completed, to avoid unnecessary Prepare
    ...
    else if (flags & asOBJ_VALUE)
    {
        ...
        else
        {
            ...
            if (flags & (asOBJ_APP_CLASS_CONSTRUCTOR | asOBJ_APP_CLASS_DESTRUCTOR | asOBJ_APP_CLASS_ASSIGNMENT |
    ...
    else
    {
        ...
        else
        {
            ...
            // TODO: Template: We don't require the lower dimensions to be registered first for registered template
            // types int[][] must not be allowed to be registered if int[] hasn't been registered first
                                             ...
                                             int compositeOffset, bool isCompositeIndirect)
{
    ...
    return RegisterBehaviourToObjectType(CastToObjectType(type.GetTypeInfo()), behaviour, decl, funcPointer, callConv,
                                                        ...
                                                        asCModule* requestingModule)
{
            ...
            type->templateSubTypes == subTypes)
        {
            ...
            if (requestingModule && generatedTemplateTypes.Exists(type))
            {
                if (type->module == 0)
                {
                    ...
                    // It may be without ownership if it was previously created from application with for example
                    // GetTypeInfoByDecl
    ...
    // The object types in templateInstanceTypes that are not also in generatedTemplateTypes are registered template
    // specializations
...
// templateSubTypes is the list of template subtypes in the original declaration, used to find the index into the target
// type list for replacement targetSubTypes is the list of target subtypes in the instance (optional) mod is the module
// that should be used if a new template instance is needed (optional) tmpl is the registered template object. Used to
// find which subtype is being replaced (optional) ot is the new template object instance that is being created. Used to
    ...
    if (factory->listPattern)
    {
        ...
        while (n)
        {
            ...
            if (newNode->type == asLPT_TYPE)
            {
                ...
                typeNode->dataType = DetermineTypeForTemplate(typeNode->dataType, templateType->templateSubTypes,
                                                                ...
                                                                asCScriptFunction* func, asCScriptFunction** newFunc)
{
    ...
    */
    ...
    func2->returnType = DetermineTypeForTemplate(func->returnType, templateType->templateSubTypes, ot->templateSubTypes,
                                                 ot->module, templateType, ot);
    func2->parameterTypes.SetLength(func->parameterTypes.GetLength());
    for (asUINT p = 0; p < func->parameterTypes.GetLength(); p++)
        func2->parameterTypes[p] = DetermineTypeForTemplate(func->parameterTypes[p], templateType->templateSubTypes,
                                                            ot->templateSubTypes, ot->module, templateType, ot);
...
void* asCScriptEngine::CreateScriptObject(const asITypeInfo* type)
{
    ...
    {
        // TODO: How to report the reason the object couldn't be created, without writing to the message callback?
        // optional argument with return code?
    ...
    else
    {
        ...
        if (objType->beh.construct == 0 && !(objType->flags & asOBJ_POD))
        {
            // TODO: How to report the reason the object couldn't be created, without writing to the message callback?
            // optional argument with return code?
            // TODO: Warn about the invalid call to message callback. Make it an optional, so the warning can be turned
            // off
            //			asCString str;
            //			str.Format(TXT_FAILED_IN_FUNC_s_s_d, "CreateScriptObject", errorNames[-asNO_FUNCTION],
            //asNO_FUNCTION); 			WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
            return 0;
        }
...
void asCScriptEngine::AddScriptFunction(asCScriptFunction* func)
{
    ...
    {
        // The slot should be empty or already set with the function, which happens if an existing shared function is
        // reused
