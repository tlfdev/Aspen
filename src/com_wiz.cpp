...
bool CMDForce::Execute(const std::string& verb, Player* mobile, std::vector<std::string>& args, int subcmd)
{
    ...
    if (BitIsSet(mobile->GetRank(), RANK_GOD) || (BitIsSet(mobile->GetRank(), RANK_ADMIN)))
    {
...
bool CMDPaste::Execute(const std::string& verb, Player* mobile, std::vector<std::string>& args, int subcmd)
{
    ...
    mobile->Message(MSG_INFO, "Entering editor. Type a . on a blank line to finish.");
    if (!TextBlockHandler::CreateHandler(
            mobile->GetSocket(),
            std::bind(&CMDPaste::TextInput, this, std::placeholders::_1, std::placeholders::_2, (void*)mobile), lines))
    {
...
bool CMDScriptStats::Execute(const std::string& verb, Player* mobile, std::vector<std::string>& args, int subcmd)
{
    ...
    vector<string> columns = {"global function count",
                              to_string(base->GetGlobalFunctionCount()),
                              "global property count",
                              to_string(base->GetGlobalPropertyCount()),
                              "object type count",
                              to_string(base->GetObjectTypeCount()),
                              "enum count",
                              to_string(base->GetEnumCount()),
                              "function definition count",
                              to_string(base->GetFuncdefCount()),
                              "typedef count",
                              to_string(base->GetTypedefCount()),
                              "module count",
                              to_string(base->GetModuleCount()),
                              "Garbage Collection Stats:",
                              "",
                              "current size",
                              to_string(currentSize),
                              "total destroyed",
                              to_string(totalDestroyed),
                              "total detected",
                              to_string(totalDetected),
                              "new objects",
                              to_string(newObjects),
                              "total new destroyed",
                              to_string(totalNewDestroyed)};
